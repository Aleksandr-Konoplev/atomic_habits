...
class Habit(models.Model):
    ...
    time = models.TimeField(verbose_name='Время в которое выполняется привычка', blank=True, null=True)
    ...
    related_pleasant_habit = models.ForeignKey(
        'habits.Habit', on_delete=models.SET_NULL, blank=True, null=True, verbose_name='Связанная приятная привычка'
    )

    periodicity = models.PositiveIntegerField(default=1, verbose_name='Периодичность выполнения привычки (в днях)')

    award = models.CharField(
        max_length=1000, blank=True, null=True, verbose_name='Вознаграждение за полезную привычку'
    )
